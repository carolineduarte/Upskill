...
public class FuncionariosController {
    ...
    @RequestMapping(value = "/funcionarios",method = RequestMethod.POST,consumes = MediaType.APPLICATION_XML_VALUE,produces = MediaType.APPLICATION_XML_VALUE)
    ...
    public ResponseEntity<Object> removeFuncionario(@PathVariable("id") int nr) {
        try {FuncionariosService.removeFuncionario(nr);
            return new ResponseEntity<>(HttpStatus.OK);
        }catch (Exception e) {
            return new ResponseEntity<>(new ErroDTO(e), HttpStatus.CONFLICT);
        }
